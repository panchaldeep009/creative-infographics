...
import data, { Movie } from './../test-data/movies';
...
export const MoviesAndGenres: Story = () => {
  ...
  const handleSelectedMovies = (title?: string) => {
    setSelectedMovie(data.find(({ Title }) => Title?.length && Title === title));
  }
  ...
  return <div style={{ display: 'flex', width: '100%' }}>
    <div style={{ height: '100vh', flex: 1 }}>
      <Graph 
        ...
        onHoverLabel={handleSelectedMovies}
      />
    </div>
    <div style={{ width: 300, padding: 10 }}>
      {selectedMovie && <MoviesInfo movie={selectedMovie} />}
    </div>
  </div>
